...
@implementation DPTimeLineItem


@end

@interface DTTimeLineView()
...
//        self.dataArrM = [NSMutableArray new];
...
- (void)longPressAction:(UILongPressGestureRecognizer*)gesture {
    ...
    //越界控制
    if (tempPoint.x >= kScreenWidth) {
        tempPoint = CGPointMake(kScreenWidth, tempPoint.y);
    }
    if (tempPoint.x == kScreenHeight - 60) {
        tempPoint = CGPointMake(tempPoint.x, kScreenHeight-60);
    }
    if (tempPoint.y <= 0.0) {
...
- (void)drawTimeLine  {
    if (self.isNeedBackGroundColor) {
        [self.timeLinePath addLineToPoint:CGPointMake(self.currentPoint.x , kScreenHeight - 60)];
...
- (void)drawFramework {
    ...
    //三条横线
    CGFloat rowSpace = (kScreenHeight -60.0)/2.0;
    NSString *tempStr = @"";
    for (int i = 0; i < 3; i ++) {
        [path moveToPoint:CGPointMake(0,rowSpace*i)];
        [path addLineToPoint:CGPointMake(kScreenWidth, rowSpace*i)];
        if (0 == i) {
    ...
    //4条竖线
    CGFloat colSpace = kScreenWidth / 4.0;
    for (int i = 0; i < 5; i ++) {
        [path moveToPoint:CGPointMake(colSpace*i,0)];
        [path addLineToPoint:CGPointMake(colSpace*i, kScreenHeight-60)];
        if (0 == i) {
            tempStr = @"9:30";
            [self drawLabelAtRect:CGRectMake(colSpace*i, kScreenHeight-60 ,30,20) textStr:tempStr];
            continue;
        }else if (1==i) {
            tempStr = @"10:30";
        }else if (2 == i) {
            tempStr = @"11:30/13:00";
            [self drawLabelAtRect:CGRectMake(colSpace*i-30, kScreenHeight-60 ,70,20) textStr:tempStr];
            continue;
        }else if (3==i) {
            tempStr = @"14:00";
        }else if (4 == i) {
            tempStr = @"15:00";
            [self drawLabelAtRect:CGRectMake(colSpace*i - 30, kScreenHeight-60 ,30,20) textStr:tempStr];
            continue;
        }
        [self drawLabelAtRect:CGRectMake(colSpace*i-15, kScreenHeight-60 ,35,20) textStr:tempStr];
    }
...
- (void)drawCrossLineWithPoint:(CGPoint)point {
    ...
    [path addLineToPoint:CGPointMake(point.x, kScreenHeight - 60)];
    [path moveToPoint:CGPointMake(0, point.y)];
    [path addLineToPoint:CGPointMake(kScreenHeight, point.y)];
    ...
    [self drawCrossLabelWithTextLayer:self.crossTimeLayer AtRect:CGRectMake(point.x, kScreenHeight-60, 30, 20) textStr:time];
...
- (void)preworkForData {
    ...
    for (int i = 0; i< self.dataArrM.count; i ++ ) {
        ...
        if (i == 0) {
            if (self.isNeedBackGroundColor) {
                [self.timeLinePath moveToPoint:CGPointMake(0, kScreenHeight - 60)];
...
-(CGFloat)miniteTimeWithTimeStr:(NSString*)timeStr {
    ...
    //每分钟代表的宽度
    CGFloat aveWidth = kScreenWidth/ 240.0;
    if (minte <= 11*60+30) {//上午
        return aveWidth*(minte - 9*60-30);
    }else {//下午
        return kScreenWidth *0.5 + aveWidth*(minte - 13*60);
    }
}
/**根据坐标点获取对应时间*/
-(NSString*)timeWithPoint:(CGPoint)point{
    NSString *str = @"";
    //单位距离代表的时间
    CGFloat aveWidth = 240.0/kScreenWidth;
...
- (NSString*)pricWithPoint:(CGPoint)point {
    NSString *price = @"";
    //单位距离代表的价格
    CGFloat avePrice = (self.max - self.min)/(kScreenHeight - 60);
    //保留两位小数
