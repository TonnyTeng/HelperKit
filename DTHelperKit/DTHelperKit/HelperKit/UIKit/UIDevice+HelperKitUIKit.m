...
@implementation UIDevice (HelperKitUIKit)
...
+ (NSString *)macAddress {
    ...
    NSString *outstring = [NSString stringWithFormat:@"%02X:%02X:%02X:%02X:%02X:%02X",//格式控制: 以十六进制输出,2为指定的输出字段的宽度.如果位数小于2,则左端补0
...
+ (NSString *) uniqueBundleIdentifier{
    ...
    NSString *uniqueIdentifier = [stringToHash toMD5];
...
- (NSString *) uniqueGlobalIdentifier{
    NSString *macaddress = [UIDevice macAddress];
    NSString *uniqueIdentifier = [macaddress toMD5];
